...
#         for m in self.modules():
#             if isinstance(m, nn.Conv2d):
#                 conv_weight_lists.append(m.weight.cpu().detach().numpy())
#         return conv_weight_lists
#
#     def get_conv_weights_mean(self):
#         # get frobenius norm (전체 element들을 다 더하는거라 mean, variance를 구할 순 없음)
#         conv_weight_lists = self.get_conv_weights()
#         result = {}
#         for idx, weight in enumerate(conv_weight_lists):
#             result[f"conv{idx}_mean"] = np.mean(weight)
#
#     def get_conv_weights_var(self):
#         # get frobenius norm (전체 element들을 다 더하는거라 mean, variance를 구할 순 없음)
#         conv_weight_lists = self.get_conv_weights()
#         result = {}
#         for idx, weight in enumerate(conv_weight_lists):
#             result[f"conv{idx}_var"] = np.var(weight)
#
#     def get_conv_weights_norm(self):
#         # get frobenius norm (전체 element들을 다 더하는거라 mean, variance를 구할 순 없음)
#         conv_weight_lists = self.get_conv_weights()
#         result = {}
#
#         for idx, weight in enumerate(conv_weight_lists):
#             result[f"conv{idx}_norm"] = np.linalg.norm(weight)
#
#         return result
#
#
...
def resnet50(pretrained=False, progress=True, **kwargs):
    ...
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, progress,
                   **kwargs)
>>>>>>> f3ba04cdf6dca44c3f19edc2286a9d1125fba889
